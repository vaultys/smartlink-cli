...
export class Brave {
  ...
  async installExtensionViaRegistry(extensionId: string): Promise<void> {
    ...
    try {
      ...
      const regCommand = `REG ADD "HKLM\\Software\\Policies\\BraveSoftware\\Brave\\ExtensionInstallForcelist" /v 1 /t REG_SZ /d "${extensionId};https://clients2.google.com/service/update2/crx" /f`;
      ...
      try {
        const regCommandUser = `REG ADD "HKCU\\Software\\Policies\\BraveSoftware\\Brave\\ExtensionInstallForcelist" /v 1 /t REG_SZ /d "${extensionId};https://clients2.google.com/service/update2/crx" /f`;
  ...
  async downloadExtension(extensionId: string): Promise<string> {
    ...
    return new Promise((resolve, reject) => {
      if (!this.options.downloadDir) {
        reject(
          new Error("Le chemin du dossier de téléchargement n'est pas défini")
        );
        return;
      }
  ...
  async openBraveWithConfigUrl(
    ...
    try {
      ...
      await BrowserController.openWithUrl("brave", configUrl, profileName);

      this.log(
        "debug",
        "Brave fermé après chargement de l'URL de configuration"
      );
    } catch (error) {
      this.log(
        "error",
        `Échec de l'ouverture de Brave avec l'URL de configuration: ${error}`
      );
    }
  }
  ...
  getBravePath(): string {
    ...
    if (!fs.existsSync(bravePath)) {
      throw new Error("Brave n'est pas installé ou le chemin est incorrect");
    }

    return bravePath;
  }

  getBraveProfiles(): string[] {
    const bravePath = this.getBravePath();

    const profiles = fs
      .readdirSync(bravePath)
      .filter((dir) => dir.startsWith("Profile") || dir === "Default")
      .map((profile) => path.join(bravePath, profile));

    if (profiles.length === 0) {
      throw new Error("Aucun profil Brave trouvé");
    }
  ...
  getProfiles(): string[] {
    let profiles = this.getBraveProfiles();
    if (!this.options.profiles || this.options.profiles.length === 0)
      return profiles;
    const bravePath = this.getBravePath();
    const localStateFile = path.join(bravePath, "Local State");
  ...
  async installExtension(extensionId: string): Promise<void> {
    try {
      await BrowserController.close("brave");
      ...
      if (os.platform() === "win32") {
        ...
        // Ouvrir Brave avec l'URL de configuration si les paramètres sont fournis
        if (this.options.config?.smartLinkUrl && this.options.config?.secret) {
          this.log("info", `Ouverture de Brave avec l'URL de configuration`);
          await this.openBraveWithConfigUrl("Default", {
        ...
        this.log(
          "info",
          "Installation terminée. Veuillez redémarrer Brave pour activer l'extension."
        );
        return;
      }
      ...
      this.log(
        "info",
        `Installation de l'extension ${extensionId} pour ${profiles.length} profils Brave...`
      );
      ...
      try {
        ...
        manifest = {
          version: "1.0.0",
          name: "VaultySid",
          description: "VaultySid Brave Extension",
          manifest_version: 3,
      ...
      for (const profile of profiles) {
        ...
        if (fs.existsSync(preferenceFile)) {
          ...
          // Générer le nom du dossier de version au format Brave (1.0.0 -> 1_0_0_0)
          ...
          if (
            ...
            this.log(
              "info",
              `Ouverture de Brave avec l'URL de configuration pour le profil: ${path.basename(
                profile
              )}`
            );

            await this.openBraveWithConfigUrl(path.basename(profile), {
              smartLinkUrl: this.options.config.smartLinkUrl,
              secret: this.options.config.secret,
            });
          }
      ...
      this.log("error", `Erreur dans Brave.installExtension: ${error}`);
