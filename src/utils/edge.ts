...
export class Edge {
  ...
  async installExtensionViaRegistry(extensionId: string): Promise<void> {
    ...
    try {
      ...
      const regCommand = `REG ADD "HKLM\\Software\\Policies\\Microsoft\\Edge\\ExtensionInstallForcelist" /v 1 /t REG_SZ /d "${extensionId};https://clients2.google.com/service/update2/crx" /f`;
      ...
      try {
        const regCommandUser = `REG ADD "HKCU\\Software\\Policies\\Microsoft\\Edge\\ExtensionInstallForcelist" /v 1 /t REG_SZ /d "${extensionId};https://clients2.google.com/service/update2/crx" /f`;
  ...
  async downloadExtension(extensionId: string): Promise<string> {
    ...
    return new Promise((resolve, reject) => {
      if (!this.options.downloadDir)
        return reject(
          new Error("Le répertoire de téléchargement n'est pas défini.")
        );
  ...
  async openEdgeWithConfigUrl(
    ...
    try {
      ...
      await BrowserController.openWithUrl("edge", configUrl, profileName);

      this.log("info", "Edge fermé après chargement de l'URL de configuration");
    } catch (error) {
      this.log(
        "error",
        `Échec de l'ouverture de Edge avec l'URL de configuration: ${error}`
      );
    }
  }
  ...
  getEdgePath(): string {
    ...
    if (process.platform === "darwin") {
      ...
      // Linux
      edgePath = path.join(this.userHomeDir, ".config", "microsoft-edge");
    }

    if (!fs.existsSync(edgePath)) {
      throw new Error("Edge n'est pas installé ou le chemin est incorrect");
    }

    return edgePath;
  }

  getEdgeProfiles(): string[] {
    const edgePath = this.getEdgePath();

    const profiles = fs
      .readdirSync(edgePath)
      .filter((dir) => dir.startsWith("Profile") || dir === "Default")
      .map((profile) => path.join(edgePath, profile));

    if (profiles.length === 0) {
      throw new Error("Aucun profil Edge trouvé");
    }
  ...
  getProfiles(): string[] {
    let profiles = this.getEdgeProfiles();
    if (this.options.profiles && this.options.profiles.length > 0) {
      const edgePath = this.getEdgePath();
      const localStateFile = path.join(edgePath, "Local State");
  ...
  async installExtension(extensionId: string): Promise<void> {
    try {
      await BrowserController.close("edge");
      ...
      if (os.platform() === "win32") {
        ...
        // Ouvrir Edge avec l'URL de configuration si les paramètres sont fournis
        if (this.options.config?.smartLinkUrl && this.options.config?.secret) {
          this.log("info", `Ouverture de Edge avec l'URL de configuration`);
          await this.openEdgeWithConfigUrl("Default", {
        ...
        this.log(
          "info",
          "Installation terminée. Veuillez redémarrer Edge pour activer l'extension."
        );
        return;
      }
      ...
      this.log(
        "info",
        `Installation de l'extension ${extensionId} pour ${profiles.length} profils Edge...`
      );
      ...
      try {
        ...
        manifest = {
          version: "1.0.0",
          name: "VaultySid",
          description: "VaultySid Edge Extension",
          manifest_version: 3,
      ...
      for (const profile of profiles) {
        ...
        // Générer le nom du dossier de version au format Edge (1.0.0 -> 1_0_0_0)
        ...
        if (fs.existsSync(preferenceFile)) {
          ...
          if (
            ...
            this.log(
              "info",
              `Ouverture de Edge avec l'URL de configuration pour le profil: ${path.basename(
                profile
              )}`
            );

            await this.openEdgeWithConfigUrl(path.basename(profile), {
              smartLinkUrl: this.options.config.smartLinkUrl,
              secret: this.options.config.secret,
            });
          }
      ...
      this.log("error", `Erreur dans Edge.installExtension: ${error}`);
