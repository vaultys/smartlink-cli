...
export class Chrome {
  ...
  async installExtensionViaRegistry(extensionId: string): Promise<void> {
    ...
    try {
      ...
      const regCommand = `REG ADD "HKLM\\Software\\Policies\\Google\\Chrome\\ExtensionInstallForcelist" /v 1 /t REG_SZ /d "${extensionId};https://clients2.google.com/service/update2/crx" /f`;
      ...
      try {
        const regCommandUser = `REG ADD "HKCU\\Software\\Policies\\Google\\Chrome\\ExtensionInstallForcelist" /v 1 /t REG_SZ /d "${extensionId};https://clients2.google.com/service/update2/crx" /f`;
  ...
  async openChromeWithConfigUrl(
    ...
    try {
      ...
      await BrowserController.openWithUrl("chrome", configUrl, profileName);

      this.log(
        "debug",
        "Chrome fermé après chargement de l'URL de configuration"
      );
    } catch (error) {
      this.log(
        "error",
        `Échec de l'ouverture de Chrome avec l'URL de configuration: ${error}`
      );
    }
  }
  ...
  getChromePath(): string {
    ...
    if (process.platform === "darwin") {
      ...
      // Linux
      chromePath = path.join(this.userHomeDir, ".config", "google-chrome");
    }

    if (!fs.existsSync(chromePath)) {
      throw new Error("Chrome n'est pas installé ou le chemin est incorrect");
    }
  ...
  getChromeProfiles(): string[] {
    ...
    const profiles = fs
      ...
      .map((profile) => path.join(chromePath, profile));

    if (profiles.length === 0) {
      throw new Error("Aucun profil Chrome trouvé");
    }
  ...
  getProfiles(): string[] {
    let profiles = this.getChromeProfiles();
    if (!this.options.profiles || this.options.profiles.length === 0)
      return profiles;
    const chromePath = this.getChromePath();
    const localStateFile = path.join(chromePath, "Local State");
  ...
  async installExtension(extensionId: string): Promise<void> {
    try {
      await BrowserController.close("chrome");
      ...
      if (process.platform === "win32") {
        ...
        // Ouvrir Chrome avec l'URL de configuration si les paramètres sont fournis
        if (this.options.config?.smartLinkUrl && this.options.config?.secret) {
          this.log("info", `Ouverture de Chrome avec l'URL de configuration`);
          await this.openChromeWithConfigUrl("Default", {
        ...
        this.log(
          "info",
          "Installation terminée. Veuillez redémarrer Chrome pour activer l'extension."
        );
        return;
      }
      ...
      this.log(
        "info",
        `Installation de l'extension ${extensionId} pour  ${profiles.length} profils Chrome...`
      );
      ...
      for (const profile of profiles) {
        ...
        if (fs.existsSync(preferenceFile)) {
          ...
          // Générer le nom du dossier de version au format Chrome (1.0.0 -> 1_0_0_0)
          ...
          if (
            ...
            this.log(
              "info",
              `Ouverture de Chrome avec l'URL de configuration pour le profil: ${path.basename(
                profile
              )}`
            );

            await this.openChromeWithConfigUrl(path.basename(profile), {
              smartLinkUrl: this.options.config.smartLinkUrl,
              secret: this.options.config.secret,
            });
          }
      ...
      this.log("error", `Erreur dans Chrome.installExtension: ${error}`);
